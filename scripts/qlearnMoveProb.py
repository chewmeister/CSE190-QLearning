	...
	while(iterationCount != maxIterations):
		...
		if( randMove == "N"):
			...
			if (newState[0] == -1 or newState[0] == map_size[0] or newState[1] == -1 or newState[1] == map_size[1]):
				...
				ForwardMaxQNext = rewardWall + curr.getValue()
				(x,y) = state
				nextState = state
			else:
				(x,y) = newState
				...
				if(nodeDirection.boolWall()):
					ForwardMaxQNext = rewardWall + (qDiscount*curr.getValue())
					nextState = state
				else:
					ForwardMaxQNext = rewardStep + (qDiscount*values[(x,y)].getValue())
					nextState = newState
			...
			if (newState[0] == -1 or newState[0] == map_size[0] or newState[1] == -1 or newState[1] == map_size[1]):
				# You've hit a wall and you're back at your own tile.
				BackwardMaxQNext = rewardWall + curr.getValue()
			else:
				(x,y) = newState
				nodeDirection = values[(x,y)]
				if(nodeDirection.boolWall()):
					BackwardMaxQNext = rewardWall + (qDiscount*curr.getValue())
				else:
					BackwardMaxQNext = rewardStep + (qDiscount*values[(x,y)].getValue())

			# Right
			newState = [state[0] + 0, state[1] + 1]
			if (newState[0] == -1 or newState[0] == map_size[0] or newState[1] == -1 or newState[1] == map_size[1]):
				# You've hit a wall and you're back at your own tile.
				RightMaxQNext = rewardWall + curr.getValue()
			else:
				(x,y) = newState
				nodeDirection = values[(x,y)]
				if(nodeDirection.boolWall()):
					RightMaxQNext = rewardWall + (qDiscount*curr.getValue())
				else:
					RightMaxQNext = rewardStep + (qDiscount*values[(x,y)].getValue())
			...
			if (newState[0] == -1 or newState[0] == map_size[0] or newState[1] == -1 or newState[1] == map_size[1]):
				# You've hit a wall and you're back at your own tile.
				LeftMaxQNext = rewardWall + curr.getValue()
			else:
				(x,y) = newState
				nodeDirection = values[(x,y)]
				if(nodeDirection.boolWall()):
					LeftMaxQNext = rewardWall + (qDiscount*curr.getValue())
				else:
					LeftMaxQNext = rewardStep + (qDiscount*values[(x,y)].getValue())
		...
		maxQNext = (pForward*ForwardMaxQNext) + (pBackward*BackwardMaxQNext) + (pRight*RightMaxQNext) + (pLeft*LeftMaxQNext)
